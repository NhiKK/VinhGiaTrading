<%@ Page Language="C#" AutoEventWireup="true" CodeFile="inc-donggoi-sanpham.aspx.cs" Inherits="inc_donggoi_sanpham" %>

<%@ Register Src="jqGrid.ascx" TagName="jqGrid" TagPrefix="uc1" %>
<script>
    function ChangeStatus() {
        var md_donggoi_id = $('#grid_ds_donggoi').jqGrid('getGridParam', 'selrow');
    ...
    "</div>");
        ...
        $("#dialog-changestatus").dialog({
            ...
            , buttons: [
            {
                ...
                text: "Thay đổi",
                click: function () {
                    var pagram = $('#grid_ds_donggoi').jqGrid('getGridParam', 'postData');
                    ...
                    if (typeof statusType != 'undefined') {
                        ...
                        $.get('Controllers/PackingController.ashx?action=updatestatus&md_donggoi_id=' + md_donggoi_id + '&trangthai=' + statusType, pagram, function (result) {
                            $("#dialog-changestatus").find("#caution").append(result);
                            $('#grid_ds_donggoi').jqGrid().trigger('reloadGrid');
                        });
                    } else {
    ...
    $(function () {
        $('#lay-center-donggoisanpham').parent().layout({
            north: {
                size: "50%"
                , onresize_end: function () {
                    var o = $("#lay-north-donggoisanpham").height();
                    $("#grid_ds_donggoi").setGridHeight(o - 90);
                }
            }
            , center: {
                onresize_end: function () {
                    var o = $("#lay-center-donggoisanpham").height();
                    $("#grid_ds_donggoisanpham").setGridHeight(o - 90);
                }
            }
        });
    });
    ...
    function funcPrintNT() {
        var xkId = $('#grid_ds_donggoi').jqGrid('getGridParam', 'selrow');
        let name = 'dialog-print-nk';
        let title = 'Trích xuất dữ liệu';
        let windowSize = "width=700,height=700,scrollbars=yes";
        $('body').append(`
            <div title="${title}" id="${name}">
                    ...
                    <tr>
                        ...
                        <td>
                            <fieldset style="padding:10px 0px">
                                <legend>
                                    <input id="printExcelNHDG" type="radio" name="rdoPrintType" value="printExcelNHDG" />
								    <label for="printExcelNHDG">Ngưng hoạt động theo excel (.xls) </label>
                                </legend>
								
                                <form
                                    enctype="multipart/form-data" 
                                    id="frm_printExcelNHDG"
                                    action='PrintControllers/InThongTinSanPhamQRCode/NgungHoatDongDongGoi.aspx'
                                    target="exportEX_popup"
									method="post" onsubmit="window.open('about:blank','exportEX_popup','${windowSize}');"
								>
									<input type="file" id="fileExcelNHDG" name="fileExcelNHDG" accept=".xls,.xlsx"/>
								</form>
                                <div style="text-align: center;margin-top: 8px;">
                                    <a target="_blank" href="Mẫu import/Đóng gói - hoạt động.xls" style="
                                        color: blue;
        ...
        $('#' + name).dialog({
            ...
            open: function (event, ui) {
                ...
                $('input[name="rdoPrintType"]', `#${name}`).click(function() {
					let val = $(this).val();
					if(val == 'printExcelNHDG') {
            ...
            buttons: [
            {
                ...
                text: "In",
                click: function () {
                    let printType = $('input[name=rdoPrintType]:checked', '#dialog-print-nk').val();
                    let printURL = "PrintControllers/";
                    let postfilt = $('#grid_ds_donggoi').jqGrid('getGridParam', 'postData').filters;
                    ...
                    if (typeof printType != 'undefined') {
                        ...
                        else if (printType == "printExcel") {
                            printURL += "InCXDanhMucDongGoi/InDMDG.aspx";
                            window.open(`${printURL}?md_donggoi_id=${xkId}&filters=${postfilt}`, "Chiết xuất danh mục đóng gói", windowSize);
                        }
                        else if (printType == "printExcelNHDG") {
							$('#frm_printExcelNHDG').submit();
						}
                    }
                    else {
...
<div class="ui-layout-north ui-widget-content" id="lay-north-donggoisanpham">
    <uc1:jqGrid ID="grid_ds_donggoi"
        ...
        ColModel="[
                ...
                },
                {
                    ...
                    , edittype: 'checkbox'
                    , stype: 'select'
                    , searchoptions: { value: ':;1:Có;0:Không' }
                    , align: 'center'
                    , editoptions:{ value:'True:False', defaultValue: 'True' }
                    , formatter: 'checkbox'
        ...
        OnSelectRow="function(ids) {
		                if(ids == null) {
			                ids=0;
			                if($('#grid_ds_donggoisanpham').jqGrid('getGridParam','records') &gt; 0 )
			                {
			                    var caption = $(this).jqGrid('getCell', ids, 'ten_donggoi');
			                    $('#grid_ds_donggoisanpham').jqGrid('setCaption','Đóng gói sản phẩm: ' +  caption);
				                $('#grid_ds_donggoisanpham').jqGrid('setGridParam',{url:'Controllers/PackingProductController.ashx?&packingId='+ids,page:1});
				                $('#grid_ds_donggoisanpham').jqGrid().trigger('reloadGrid');
				            } 
				        } else {
				            var caption = $(this).jqGrid('getCell', ids, 'ten_donggoi');
			                $('#grid_ds_donggoisanpham').jqGrid('setCaption', 'Đóng gói sản phẩm theo cách : ' + caption);
			                $('#grid_ds_donggoisanpham').jqGrid('setGridParam',{url:'Controllers/PackingProductController.ashx?&packingId='+ids,page:1});
			                $('#grid_ds_donggoisanpham').jqGrid().trigger('reloadGrid');			
		                } }"
...
<div class="ui-layout-center ui-widget-content" id="lay-center-donggoisanpham">
    <uc1:jqGrid ID="grid_ds_donggoisanpham"
        ...
        AddFormOptions=" 
                ...
                ,beforeShowForm: function (formid) {
                    var masterId = $('#grid_ds_donggoi').jqGrid('getGridParam', 'selrow');
...
<script>
    if (chietxuatDGSP.toLowerCase() == 'false') {
        var parent = $('#grid_ds_donggoi-pager_left');
        parent.find('.ui-icon.ui-icon-print').parent().parent().remove();
    }

    createRightPanel('grid_ds_donggoi');
    createRightPanel('grid_ds_donggoisanpham');
</script>
