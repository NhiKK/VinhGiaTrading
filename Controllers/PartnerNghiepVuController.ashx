<%@ WebHandler Language="C#" Class="PartnerNghiepVuController" %>
...
public class PartnerNghiepVuController : IHttpHandler
{
    ...
    {
        String sql = "select md_loaidtkd_id, ten_loaidtkd from md_loaidtkd where hoatdong = 1 and ma_loaidtkd in ('NHH','KH') order by ma_loaidtkd asc ";
    ...
    public void add(HttpContext context)
    {
        ...
        if (dtkd == null)
        {
            if (context.Request.Form["ma_dtkd"].Contains(" "))
                jqGridHelper.Utils.writeResult(0, "Mã đối tác phải được viết liền không chứa khoảng trắng!");
            else
            {
                string md_quocgia_id = context.Request.Form["tenqg"];
                md_quocgia qg = db.md_quocgias.FirstOrDefault(s => s.md_quocgia_id == md_quocgia_id);
                md_doitackinhdoanh mnu = new md_doitackinhdoanh
                {
                    ...
                    md_quocgia_id = qg.md_quocgia_id,
                    ...
                    diachi = context.Request.Form["diachi"] + ", " + qg.ten_quocgia,
    ...
    public void load(HttpContext context)
    {
        ...
        WHERE 1=1 and dtkd.hoatdong = 1 {0} and ldt.ma_loaidtkd in ('NHH','KH') " + filter;
