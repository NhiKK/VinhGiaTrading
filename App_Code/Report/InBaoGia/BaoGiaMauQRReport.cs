...
/// <summary>
/// Summary description for BaoGiaMauQRReport
/// </summary>
public class BaoGiaMauQRReport
{
    private String c_baogiaqr_id, logoPath, productImagePath;
    ...
    public HSSFWorkbook CreateWBQuotation()
    {
        //try
        //{
        string orderby = UserUtils.get_sapxep_qodetail(null, c_baogiaqr_id, "B.ma_sanpham", "order by B.ma_sanpham asc", db);
        ...
        String selectDetails = String.Format(@"
            ...
            from 
	            c_baogiaqr bg
				left join c_chitietbaogiaqr ctbg ON bg.c_baogiaqr_id = ctbg.c_baogiaqr_id
            ...
            where bg.c_baogiaqr_id = N'{0}')A)B {1}", c_baogiaqr_id == null ? "" : c_baogiaqr_id, orderby);
        ...
        String selectRefereces = String.Format(@"SELECT 
                                                   ...
                                                   FROM 
	                                                   c_baogiaqr bg
														left join md_color_reference color on bg.c_baogiaqr_id = color.c_baogia_id
                                                   WHERE
                                                       color.hoatdong = 1
                                                       AND bg.c_baogiaqr_id = N'{0}'", c_baogiaqr_id == null ? "" : c_baogiaqr_id);
        ...
        String selectDistinct = String.Format(@"SELECT distinct chungloai
                                                    FROM(
                                                        ...
                                                        WHERE
															color.hoatdong = 1
                                                            AND bg.c_baogiaqr_id = N'{0}'
                                                    ) as tmp", c_baogiaqr_id == null ? "" : c_baogiaqr_id);
    ...
    private HSSFWorkbook CreateHSSFWorkbook(DataTable dtDetails, DataTable dtDistinct, DataTable dtReferences)
    {
        ...
        headerColumn.GetCell(27).SetCellValue(String.Format("M2"));
        s1.AddMergedRegion(new CellRangeAddress(6, 7, 27, 27));
        ...
        s1.AddMergedRegion(new CellRangeAddress(6, 6, 28, 29));
        ...
        s1.AddMergedRegion(new CellRangeAddress(6, 7, 30, 30));
        ...
        for (int i = 0; i < detailsCount; i++)
        {
            ...
            s1.GetRow(8 + i).CreateCell(27).SetCellValue(double.Parse(dtDetails.Rows[i]["dientich"].ToString()));
            s1.GetRow(8 + i).CreateCell(28).SetCellValue(double.Parse(dtDetails.Rows[i]["inner_"].ToString()));
            s1.GetRow(8 + i).CreateCell(29).SetCellValue(double.Parse(dtDetails.Rows[i]["sl_outer"].ToString()));
            s1.GetRow(8 + i).CreateCell(30).SetCellValue(dtDetails.Rows[i]["thanhphan"].ToString());
        }
        ...
        for (int i = 0; i < dtDetails.Rows.Count; i++)
        {
            for (int j = 0; j <= 30; j++)
            {
                if (j.Equals(4) || j.Equals(5) || j.Equals(7) || j.Equals(8) || j.Equals(9) || j.Equals(10) || j.Equals(11) || j.Equals(12) || j.Equals(13) || j.Equals(14) || j.Equals(15) || j.Equals(27))
                {
                ...
                else
                {
                    ...
                    //--Size Product
                    s1.GetRow(8 + i).GetCell(24).CellStyle = cellNumBorderKT;
                    s1.GetRow(8 + i).GetCell(25).CellStyle = cellNumBorderKT;
                    s1.GetRow(8 + i).GetCell(26).CellStyle = cellNumBorderKT;
        ...
        for (int i = 0; i < dtDistinct.Rows.Count; i++)
        {
            ...
            for (int j = 0; j < items.Count; j++)
            {
                ...
                if (j == 0 || j == items.Count)
                {
                    ...
                    //s1.GetRow(9 + detailsCount + curLine * 8 + i * 8).CreateCell(1).SetCellValue(dtDistinct.Rows.Count + "");
    ...
    public String C_baogiaqr_id
    {
        get { return c_baogiaqr_id; }
        set { c_baogiaqr_id = value; }
    }

    public BaoGiaMauQRReport(String c_baogiaqr_id, String logoPath, String productImagePath)
    {
        this.c_baogiaqr_id = c_baogiaqr_id;
