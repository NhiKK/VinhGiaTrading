...
public partial class Tool_TuyChinhChuanBiHang : System.Web.UI.Page
{
    ...
    {   
        ...
        if (send)
        {
            try
            {
                var jsons = config.getJsonInfoCHCBH();
                var json = jsons.Where(s => s["mau"].ToString() == mau).FirstOrDefault();
                if (json != null)
                {
                    json["songay"] = songay;
                    config.writeJsonInfoCHCBH(JsonConvert.SerializeObject(jsons, Formatting.Indented));
                }
